...
```bash
npx eas-cli build --platform ios --profile production --clear-credentials --clear-cache
```

**Flags explained**:
- `--platform ios`: Build for iOS
- `--profile production`: Use production build configuration (App Store distribution)
- `--clear-credentials`: Force EAS to fetch fresh credentials and regenerate provisioning profile
